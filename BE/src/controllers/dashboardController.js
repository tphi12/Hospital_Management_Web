...
const getDashboardStats = async (req, res) => {
  try {
    ...
    const [[{ total_documents }]] = await pool.execute('SELECT COUNT(*) as total_documents FROM DOCUMENT WHERE deleted_at IS NULL');
    ...
    const [docsByDept] = await pool.execute(`
      ...
      FROM DEPARTMENT dep
      LEFT JOIN DOCUMENT d ON dep.department_id = d.department_id AND d.deleted_at IS NULL
